...
export function ProjectSharingDialog({
  ...
  useEffect(() => {
    if (!open || !baseUrl || !projectId) return;
    let cancelled = false;
    setLoading(true);
    Promise.all([
      api.getProjectSharing(baseUrl, projectId),
  ...
  async function handleTogglePublic(checked: boolean) {
    ...
    setSaving(true);
    try {
      const next = await api.updateProjectVisibility(baseUrl, projectId, checked ? "public" : "private");
      setSharing(next);
      toast.success(checked ? "Stack publica" : "Stack privada");
    } catch {
  ...
  async function handleShare() {
    if (!baseUrl || !projectId || !selectedUserId) return;
  ...
  async function handleRevoke(userId: string) {
    ...
    setSaving(true);
    try {
      await api.revokeProjectShare(baseUrl, projectId, userId);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Compartilhar stack</DialogTitle>
          <DialogDescription>{project?.name ?? "Stack"}</DialogDescription>
        </DialogHeader>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Qualquer pessoa pode abrir, editar e executar esta stack.
                  </p>
                </div>
              </div>
              <Switch
